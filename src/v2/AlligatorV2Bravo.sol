...
import {IGovernorBravoExtended} from "../interfaces/IGovernorBravoExtended.sol";
...
contract AlligatorV2Bravo is IAlligatorV2Bravo, ENSHelper, Ownable, Pausable {
    // =============================================================
    //                             ERRORS
    // =============================================================
    ...
    IGovernorBravoExtended public immutable governor;
    ...
    constructor(
        IGovernorBravoExtended _governor,
    ...
    function propose(
        ...
        proposalId = IGovernorBravoExtended(proxy).propose(targets, values, signatures, calldatas, description);
    ...
    function castVote(
        ...
        address proxy = proxyAddress(authority[0], proxyRules);
        IGovernorBravoExtended(proxy).castVote(proposalId, support);
    ...
    function castVoteWithReason(
        ...
        address proxy = proxyAddress(authority[0], proxyRules);
        IGovernorBravoExtended(proxy).castVoteWithReason(proposalId, support, reason);
        emit VoteCast(proxy, msg.sender, authority, proposalId, support);
    }
    ...
    function castVotesWithReasonBatched(
        ...
        for (uint256 i; i < authorityLength; ) {
            ...
            IGovernorBravoExtended(proxies[i]).castVoteWithReason(proposalId, support, reason);
    ...
    function castRefundableVotesWithReasonBatched(
        ...
        for (uint256 i; i < authorityLength; ) {
            ...
            validate(rules, msg.sender, authority, PERMISSION_VOTE, proposalId, support);
            proxies[i] = proxyAddress(authority[0], rules);
            IGovernorBravoExtended(proxies[i]).castRefundableVoteWithReason(proposalId, support, reason);
    ...
    function _validateRules(
        ...
        unchecked {
            ...
            if (rules.blocksBeforeVoteCloses != 0) {
                IGovernorBravoExtended.Proposal memory proposal = governor.proposals(proposalId);
