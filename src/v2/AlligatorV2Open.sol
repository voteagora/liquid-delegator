...
import "../interfaces/IAlligatorV2Open.sol";
...
contract AlligatorV2Open is IAlligatorV2Open, ENSHelper, Ownable, Pausable {
    // =============================================================
    //                             ERRORS
    // =============================================================
    ...
    IGovernorOpenExtended public immutable governor;
    ...
    constructor(
        IGovernorOpenExtended _governor,
    ...
    function propose(
        ...
        proposalId = IGovernorOpenExtended(proxy).propose(targets, values, calldatas, description);
    ...
    function castVotesWithReasonBatched(
        ...
        for (uint256 i; i < authorityLength; ) {
            ...
            IGovernorOpenExtended(proxies[i]).castVoteWithReason(proposalId, support, reason);
    ...
    function castRefundableVotesWithReasonBatched(
        ...
        for (uint256 i; i < authorityLength; ) {
            ...
            validate(rules, msg.sender, authority, PERMISSION_VOTE, proposalId, support);
            proxies[i] = proxyAddress(authority[0], rules);
            IGovernorOpenExtended(proxies[i]).castRefundableVoteWithReason(proposalId, support, reason);
